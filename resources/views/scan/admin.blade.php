...
<div class="card">
    ...
    <div class="grid grid-cols-2" style="margin-bottom: 2rem;">
        ...
        <div>
            <p style="color: var(--text-muted); margin-bottom: 0.25rem;">القسم</p>
            <p style="font-weight: 500;">{{ $equipment->department }}</p>
        </div>
        <div>
        ...
        <div>
            <p style="color: var(--text-muted); margin-bottom: 0.25rem;">تاريخ الشراء</p>
            <p style="font-weight: 500;">{{ $equipment->purchase_date ? $equipment->purchase_date->format('Y-m-d') : 'غير محدد' }}</p>
        </div>
    </div>
    ...
    <div style="display: flex; flex-direction: column; gap: 1rem;">
        ...
        <a href="{{ route('maintenance.create', ['equipment_id' => $equipment->id]) }}" class="btn btn-secondary" style="width: 100%;">
            <i class="fa-solid fa-screwdriver-wrench"></i> تسجيل صيانة جديدة
        </a>
        
        <a href="{{ url('maintenance/'.$equipment->id.'/history') }}" class="btn btn-outline" style="width: 100%;">
            <i class="fa-solid fa-clock-rotate-left"></i> سجل الصيانة
        </a>
